...
class Pdb:
    ...
    #####################################
    def GetPrimaryStructure(self, chain="", verbose=False):
        seq = self.GetAminoAcids()
        ps = ''
    ...
    def GetContactMap(self,
                      flag=['C','CA','CB','CG','CG1','CG2','CD','CD1','CD2','CE','CE1','CE2','CZ','CH','CH1','CH2',
                            ...
                            'O','OA','OB','OG','OG1','OG2','OD','OD1','OD2','OE','OE1','OE2','OZ','OH','OH1','OH2', 'OXT',
        ...
        for resi in range(nAminoAcids):
            for resj in range(resi, nAminoAcids):
                ...
                ni = self.atom[0]['iResidue'] # not all amino acids are indexed starting from 0
                first = [atomi for atomi in self.atom if atomi['iResidue'] == resi+ni]  # atoms of first amino acid
                second = [atomj for atomj in self.atom if atomj['iResidue'] == resj+ni] # atoms of second amino acid
                for i in first:
                    for j in second:

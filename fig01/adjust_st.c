...
int AdjustSTempering(struct st_stuff *p, double step)
{
	...
	if (p->tonlyadd>0)
		if (p->temp[p->ntemp-1]<p->tonlyadd)
		{
			p->removet = 0;
		}

	if (p->debug>0) fprintf(stderr,"\nADJUSTING %d TEMPERATURES (step=%.0lf, iter=%d)\n",p->ntemp,step,p->iter);
	if (p->debug>0 && p->removet>0) fprintf(stderr,"(trying to reduce the number of temperatures)\n");

	// Check if all temperatures are sampled. If not, redo simulation
	if (p->iter>0)
	{

		if (p->failure<p->nfail1)		// if fails for the first nfail1 times, fill with T optimized from reliable_lg
			j = CheckHistogramsAdjust(p->counts,p->temp,p->g,p->reliable_lg,&(p->ntemp),p->emin,p->ebin,p->nbin,p->hthresh,p->debug,p->ntempmax,p->h,
							p->prob_up,p->prob_down,p->phthresh,p->binok,p->gmethod);
		else if (p->failure<p->nfail2)	// if fails for the first nfail2 times, fill with T from extrapolated free energies
				j =CheckHistogramsAdjust3(p->counts,p->temp,p->g,&(p->ntemp),p->nbin,p->hthresh,p->debug,p->ntempmax,p->h,
						p->prob_up,p->prob_down,p->phthresh);
		else 		// if previous run failed as well, use reliable_t and reliable_g
				j = CheckHistogramsAdjust2(p->counts,p->temp,p->g,&(p->ntemp),p->hthresh,p->debug,p->reliable_t,
						p->reliable_g,&(p->nreliable_t),p->prob_up,p->prob_down,p->phthresh);


		// write on accept.dat
		fprintf(p->fpacc,"%lf %d\n",step,j); fflush(NULL);

		if (j!=1) (p->failure) ++;						// counts how many consecutive failed runs
	...
	if (p->oldh != NULL)
		AddHistogramPile(p->ntemp,p->temp,p->h,p->oldh,p->oldt,p->oldt_iter,&(p->noldt),p->nbin,p->nkeepold,p->iter,p->debug,p->sum);

	// store a reliable density of states
	if (p->iter>0)
		for (i=0;i<p->nbin;i++)
			p->reliable_lg[i] = p->current_lg[i];
	else													// in the first run, there is not a current_lg
    {
		FilterHistograms(p->h,p->htmp,p->ntemp,p->nbin,p->binthresh);
		p->reliable_lg = MHistogram(p->htmp,p->temp,p->ntemp,p->nbin,p->emin,p->ebin,p->debug,KB,p->reliable_lg,
				p->f,p->paranoid,p->ignoreb,NULL,p->deltat,p->binok);
    }
	// store reliable temperatures and weights
	for (it=0;it<p->ntemp;it++)
	{
		p->reliable_t[it] = p->temp[it];
		p->reliable_g[it] = p->g[it];
	}
	p->nreliable_t = p->ntemp;
	...
	if (p->oldh == NULL)						// uses only last histograms
	{
		OrderTemperatures(p->ntemp,p->temp,p->g,p->prob_up,p->prob_down,p->h,p->nbin,p->debug,p->counts);
		FilterHistograms(p->h,p->htmp,p->ntemp,p->nbin,p->binthresh);
		p->current_lg = MHistogram(p->htmp,p->temp,p->ntemp,p->nbin,p->emin,p->ebin,p->debug,KB,p->current_lg,
				p->f,p->paranoid, p->ignoreb,NULL,p->deltat,p->binok);
	}
	else									// uses all past histograms
	{
		OrderTemperatures(p->noldt,p->oldt,NULL,NULL,NULL,p->oldh,p->nbin,p->debug,p->oldt_iter);
		FilterHistograms(p->oldh,p->htmp,p->noldt,p->nbin,p->binthresh);
		p->current_lg = MHistogram(p->htmp,p->oldt,p->noldt,p->nbin,p->emin,p->ebin,p->debug,KB,p->current_lg,p->f,
				p->paranoid,p->ignoreb,NULL,p->deltat,p->binok);
	}
	...
	CalculateThermodynamics(p->current_lg,p->emin,p->ebin,p->nbin,tmin,tbin,NTBIN,p->out,KB,NULL,NULL,p->binok);
	PrintThermodynamics(fthe,p->out,tmin,tbin,NTBIN);
	fclose(fthe);

	// print density of states
	if (p->debug>1) fprintf(stderr,"\nWriting density of states to file...\n");
	fthe = fopen(p->nfdos,"w");
	if (!fthe) FatalError("Cannot open file for writing density of states");
	for (i=0;i<p->nbin;i++) fprintf(fthe,"%lf\t%lf\n",p->emin+p->ebin*i,p->reliable_lg[i]);
	fclose(fthe);
	...
	{
		if (p->oldh == NULL)
			PrintHistogram(p->nfhisto,p->htmp,p->temp,p->ntemp,p->nbin,p->emin,p->ebin);
		else
			PrintHistogram(p->nfhisto,p->htmp,p->oldt,p->noldt,p->nbin,p->emin,p->ebin);
	}
	...
	if (strcmp(p->nfdumb,""))
	{
		...
		if (p->oldh == NULL)
			...
			PrintAverageEnergies(fthe,p->oldh,p->oldt,p->noldt,p->emin,p->ebin,p->nbin);
		fclose(fthe);
	}

	// check if ntmax is reached, then end
	if (p->ntemp == p->ntempmax-1)
	{
	...
	OptimalWeights(p->ntemp,p->temp,p->g,p->emin,p->ebin,p->nbin,p->current_lg,p->debug,p->binok,p->gmethod);

	// adjust temperatures from 1 to ntemp-1, calculating optimal weights
	if (p->removet>0)
	{
		if (p->ntemp>3)
			p->ntemp = ReduceTemperatures(p->ntemp,p->temp,p->g,p->emin,p->ebin,p->nbin,p->debug,p->current_lg,
					p->pthresh,p->ntempmax,p->removet,p->boltzp,p->binok,p->gmethod);

		if (p->ntemp==0) return 0;
	}
	else
	{
		if (p->ntemp>2)
			MaximizeTempProb(p->ntemp,p->temp,p->g,p->emin,p->ebin,p->nbin,p->debug,p->current_lg,p->boltzp,p->binok,p->gmethod);
	}

	// add new temperature
	if (p->k>0)								// proportionally to energy sigma
		p->ntemp = AddNewTemperature(p->ntemp,p->temp,p->g,p->current_lg,p->emin,p->ebin,p->nbin,p->debug,p->k,
				p->paranoid,p->binok,p->gmethod);
	else if (p->p_new<0)						// at wished exchange probability
		p->ntemp = AddNewTemperature2(p->ntemp,p->temp,p->g,p->current_lg,p->emin,p->ebin,p->nbin,p->debug,p->p_new,
				p->paranoid,p->boltzp,p->binok,p->gmethod);

	if (p->debug>0)
	{
		fprintf(stderr,"Now there are %d temperatures:\n",p->ntemp);
		for (j=0;j<p->ntemp;j++) fprintf(stderr,"%lf (g=%lf) ",p->temp[j],p->g[j]);
		fprintf(stderr,"\nBACK TO SIMULATION\n\n");
	}

	OrderTemperatures(p->ntemp,p->temp,p->g,p->prob_up,NULL,NULL,p->nbin,p->debug,NULL);
		...
		int debug, double p_new, int paranoid, double **boltzp, int *binok, int gmethod)
{
	...
	for (it=NTBIN-1;it>0;it--)
	{
		...
		if (it == NTBIN && p1+p2<p_new+p_new)			// if the first is already below p_new
		{
	...
	if (newtemp<0)
		{
			...
			else
			{
				if (debug>0)
				{
					fprintf(stderr,"WARNING: cannot add new temperature; maybe lp_new is too small (continuing)\n");
				}
				return ntemp;
			}
		}
		...
		double **h, double *prob_up, double *prob_down, double pthresh)
{
	...
	if (nbadh>0)
	{
		...
		for (it=1;it<*ntemp;it++)
			...
			OrderTemperatures(*ntemp,temp,g,prob_up,prob_down,h,nbin,debug,counts);
...
void ManageRestart(struct st_stuff *p, double step)
{
	...
	for (i=NRESTARTS-1;i>0;i--)
		if ( ((p->st_restart)+i-1)->step >-1 )
		{
			((p->st_restart)+i)->step = ((p->st_restart)+i-1)->step;
			((p->st_restart)+i)->ntemp = ((p->st_restart)+i-1)->ntemp;
			for (j=0;j<NTEMPMAX;j++)
			{
				(((p->st_restart)+i)->temp)[j] = (((p->st_restart)+i-1)->temp)[j];
				(((p->st_restart)+i)->g)[j] = (((p->st_restart)+i-1)->g)[j];
			}
			for (j=0;j<p->nbin;j++)
				(((p->st_restart)+i)->lg)[j] = (((p->st_restart)+i-1)->lg)[j];
		}

	// write the current data on the 0-th element of the stack
	((p->st_restart)+0)->step = step;
	((p->st_restart)+0)->ntemp = p->nreliable_t;
	for (j=0;j<NTEMPMAX;j++)
	{
		(((p->st_restart)+0)->temp)[j] = p->reliable_t[j];
		(((p->st_restart)+0)->g)[j] = p->reliable_g[j];
	}
	for (j=0;j<p->nbin;j++)
		(((p->st_restart)+0)->lg)[j] = p->reliable_lg[j];
	...
	for (i=0;i<NRESTARTS;i++)
		if ( ((p->st_restart)+i)->step >-1 )
		{
			fprintf(fr,"restart %d\n",i);
			fprintf(fr,"%lf\t%d\n",((p->st_restart)+i)->step,((p->st_restart)+i)->ntemp);
			for (j=0;j<((p->st_restart)+i)->ntemp;j++) fprintf(fr,"%lf\t%lf\n",(((p->st_restart)+i)->temp)[j],(((p->st_restart)+i)->g)[j]);
			fprintf(fr,"%lf %lf %d\n",p->emin,p->ebin,p->nbin);
			for (j=0;j<p->nbin;j++) fprintf(fr,"%lf\n",(((p->st_restart)+i)->lg)[j]);
		}
